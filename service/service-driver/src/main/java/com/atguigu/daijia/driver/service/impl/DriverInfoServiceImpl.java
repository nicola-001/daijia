...
public class DriverInfoServiceImpl extends ServiceImpl<DriverInfoMapper, DriverInfo> implements DriverInfoService {
    ...
    public DriverLoginVo getDriverInfo(Long driverId) {
        log.info("获取司机登录信息：{}", driverId);
        //根据司机id获取司机信息
        DriverInfo driverInfo = driverInfoMapper.selectById(driverId);
        // 检查是否为空
        if (driverInfo == null) {
            throw new GuiguException(ResultCodeEnum.DATA_ERROR.getCode(), "未找到司机信息");
        }
    ...
    public Boolean updateDriverAuthInfo(UpdateDriverAuthInfoForm updateDriverAuthInfoForm) {
        ...
        System.out.println("获取到的司机id为：" + driverId);
        DriverInfo existingDriver = driverInfoMapper.selectById(driverId);
...
//        int i = driverInfoMapper.updateById(driverInfo);
        ...
        //判断是否插入成功，若插入失败则报错
        if (!update) {
            throw new GuiguException(ResultCodeEnum.DATA_ERROR.getCode(), "修改司机信息失败");
        }
        return update;
    }
    ...
    public Boolean isFaceRecognition(Long driverId) {
        ...
        // 若返回0 未进行人脸识别; 若返回1 已进行人脸识别
        // 返回 false 表示未进行人脸识别; 返回true  表示已进行人脸识别
        return count != 0;
    }
